...
clear tmp idx
figure(1),clf,colormap('gray'),imshow(a),title('original Lena
image') [m,n]=size(a); % size of image a
...
tmp=a(64*(i-1)+1:64*i,64*(j1)+1:64*j); tmpi=reshape(tmp,64*64,1);
mu=sort(rand(2,1)); % value betwen 0 and 1, two clusters only
[W,iter,Sw,Sb,Cova]=kmeansf(tmpi,mu);% W is the mean,
% Cova is the covariance matrices
% member: membership of each X: K by 1 vector of elements 1 to c
[d,member]=kmeantest(tmpi,sort(W)); c(64*(i-1)+1:64*i,64*(j1)+1:64*j)=reshape(member,64,64);
end
end
figure(4),clf,colormap('gray'),imshow(c1), title('local threshold, 64x64 block');